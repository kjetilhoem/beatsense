...
class SpectralCentroidAnalyzerTest {
    ...
    private val analyzer = SpectralCentroidAnalyzer()
    ...
    @Test
    fun `silence returns dash`() {
        // Feed a few frames of silence to get past Pending
        repeat(10) {
    ...
    @Test
    fun `low frequency signal has low centroid`() {
        // 100 Hz sine wave — should be in the "Dark" or "Warm" range
    ...
    @Test
    fun `high frequency signal has high centroid`() {
        // 8000 Hz sine wave — should be "Bright" or "Harsh"
        ...
        assertTrue(result is AnalyzerResult.HeroValue)
        val value = (result as AnalyzerResult.HeroValue).value
        assertTrue("High freq should be Bright or Harsh, got: $value", value.contains("Bright") || value.contains("Harsh"))
    }

    @Test
    fun `label is BRIGHTNESS`() {
        val pcm = FloatArray(bufferSize) { sin(2f * PI.toFloat() * 440f * it / sampleRate) * 0.5f }
        val result = analyzer.analyze(AudioFrame(pcm))
        when (result) {
            is AnalyzerResult.HeroValue -> assertEquals("BRIGHTNESS", result.label)
            is AnalyzerResult.Pending -> assertEquals("BRIGHTNESS", result.label)
